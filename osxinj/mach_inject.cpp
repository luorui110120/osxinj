            ...
            vm_size_t				stackSize )
{
	...
	if( !err ) {
		assert( (unsigned long)threadEntry >= (unsigned long)image && (unsigned long)threadEntry <= ((unsigned long)image+imageSize) );
		ASSERT_CAST( void*, threadEntry );
		threadEntryOffset = ((unsigned long) threadEntry) - (unsigned long)image;
